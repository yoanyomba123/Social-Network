    ...
    User.findOne({ username: req.user.username })
      ...
      .exec(function(err, user) {
        ...
        notificationFeed.get({ limit: 0 }).then(function(body) {
          if (typeof body != "undefined") request.user.unseen = body.unseen;
          next();
        });
      });
  } else {
...
router.get("/View/user/:id", ensureAuthenticated, users.getSomeUserProfile);
...
router.get("/test", ensureAuthenticated, function(request, response) {
  response.render("../views/Profile/User/UserTemplate");
});
